<script>
  ...
  let submit = async () => {
    ...
    try {
      await register(email, username, password, promo);
      registered = true;
    } catch (e) {
      err(e);
    }
...
<div class="form-container bg-lightblue px-4">
  <form class="mb-6" on:submit|preventDefault={submit} autocomplete="off">
    ...
    {:else if registered}
      ...
      <p>Thanks for registering! You may now log in with your new account.</p>
    ...
    {:else}
      ...
      <div class="flex flex-col mb-4">
        ...
        <div class="flex flex-col mb-4">
          <label class="mb-2 font-medium text-gray-600" for="promo"
            >Promo code</label
          >
          <input
            id="promo"
