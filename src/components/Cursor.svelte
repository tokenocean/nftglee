  ...
  onMount(() => {
    function edgtfThemeCursor() {
      ...
      var hoverClass = function () {
        var items =
          "a, button, .tp-bullet, .tp-withaction, button, .edgtf-pl-filter, .ui-slider-handle, .edgtf-btn .items-center a";
        var addCSSClass = function () {
          !cursor.hasClass("edgtf-hovering") &&
            cursor.addClass("edgtf-hovering");
        };

        var removeCSSClass = function () {
          cursor.hasClass("edgtf-hovering") &&
            cursor.removeClass("edgtf-hovering");
        };
      ...
      var hideCursor = function () {
        cursor.hasClass("edgtf-visible") &&
          cursor.removeClass("edgtf-visible edgtf-hovering");
      };
      ...
      var controlItems = function () {
        var items = jquery(
          ".edgtf-video-button-play, .edgtf-btn-circle, .edgtf-pl-more, .edgtf-pl-load-more, .edgtf-page-not-found svg"
        );
        ...
        items.length &&
          items.each(function () {
            ...
            var updatePosition = function () {
              ...
              inRange =
                Math.abs(x - cX) < w * coeff && Math.abs(y - cY) < h * coeff;
            };
            ...
            var coords = function () {
              return {
                x:
                  Math.abs(cX - x) < limit
                    ? cX - x
                    : (limit * (cX - x)) / Math.abs(cX - x),
                y:
                  Math.abs(cY - y) < limit
                    ? cY - y
                    : (limit * (cY - y)) / Math.abs(cY - y),
              };
            };
        ...
        var instances = [
            {
              type: "flame",
              triggers:
                ".edgtf-portfolio-list-holder .edgtf-pli-link, .edgtf-plh-item > a",
            },
            {
              type: "cart",
            ...
            },
            {
              type: "eye",
              triggers:
                ".edgtf-image-behavior-two-links-overlay .edgtf-iwt-image a, .edgtf-image-behavior-custom-link .edgtf-iwt-image",
            },
            {
              type: "close",
              triggers:
                ".edgtf-fullscreen-search-holder, .edgtf-fullscreen-menu-holder, .edgtf-cover",
            },
          ],
          triggers = "",
          hides =
            ".edgtf-portfolio-info-float .edgtf-pli-link, .edgtf-pls-item-inner, .fluidvids",
          overrides =
            "form, #menu-full-screen-menu, .edgtf-portfolio-slider-holder .edgtf-pl-standard-shader .edgtf-pli-text-holder";
      ...
      var blinkClass = function () {
        jquery(document).on("click", "a", function (e) {
          ...
          if (
            ...
            cursor.removeClass().addClass("edgtf-blink");
          }
        });
      };
